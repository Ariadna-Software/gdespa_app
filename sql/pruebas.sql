SELECT 1 AS storeId, it.itemId, (COALESCE(i.qin, 0) -  COALESCE(o.qout, 0)) AS stock 
FROM item AS it 
LEFT JOIN 
(SELECT iin.storeId, iinl.itemId, SUM(iinl.quantity) AS qin 
FROM item_in AS iin 
LEFT JOIN item_in_line AS iinl ON iinl.itemInId = iin.itemInId 
WHERE iin.storeId = 1 AND iinl.itemId = 240 AND iin.dateIn > '2016-09-17 00:00:00.000' 
...
WHERE iout.storeId = 1 AND ioutl.itemId = 240 
AND iout.dateOut > '2016-09-17 00:00:00.000' 
