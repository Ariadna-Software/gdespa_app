    ...
    postGenerated: function (pl, done, test) {
        ...
        dbCon.getConnection(function (err, res) {
            ...
            con.query(sql, function (err, res) {
                if (err) return done(err);
                pl.plId = res.insertId;
                sql = "INSERT INTO pl_line (plId, cunitId, estimate, done, quantity, chapterId, pwLineId, prevPlanned)";
                sql += " SELECT ";
                sql +=  pl.plId + " AS plId, pwl.cunitId, pwl.quantity AS estimate, calc.done, 0,  pwl.chapterId, pwl.pwLineId, pln.prevPlanned";
