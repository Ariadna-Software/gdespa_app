...
var workerAPI = {
    ...
    getHours: function (fromDate, toDate, workerId, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT v.workerId AS vWokId, v.workerName AS vName, COALESCE(d.wDayType, 'SIN DATOS') AS vDay, DATE_FORMAT(v.vdate,'%Y-%m-%d') AS vDate, WEEK(v.vdate, 1) AS vWeek, d.* FROM ";
            sql += " (SELECT w.workerId, w.name AS workerName, d1.vdate  FROM ";
