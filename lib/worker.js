...
var workerAPI = {
    ...
    getWorkerActive: function (done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT w.*, u.name as userName, r.name as resName";
            ...
            sql += " LEFT JOIN user as u ON u.userId = w.userId"
            sql += " LEFT JOIN resource_type as r ON r.resTypeId = w.resTypeId";
            sql += " WHERE w.resTypeId = 0 AND w.active = 1"
            con.query(sql, function (err, res) {
                dbCon.closeConnection(con);
                if (err) return done(err);
                var workers = [];
    ...
    getVehicleActive: function (done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT w.*, u.name as userName, r.name as resName";
            ...
            sql += " LEFT JOIN user as u ON u.userId = w.userId"
            sql += " LEFT JOIN resource_type as r ON r.resTypeId = w.resTypeId";
            sql += " WHERE w.resTypeId = 1 AND w.active = 1"
            con.query(sql, function (err, res) {
                dbCon.closeConnection(con);
                if (err) return done(err);
                var workers = [];
