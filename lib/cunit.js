...
// -------------------- MAIN API
var cunitAPI = {
    get: function (block, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT i.* FROM cunit as i WHERE block = 0";
            if (block) sql = "SELECT i.* FROM cunit as i";
    ...
    getBlock: function (done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT i.* FROM cunit as i WHERE block = 1";
            con.query(sql, function (err, res) {
                dbCon.closeConnection(con);
                if (err) return done(err);
                var cunits = [];
