    ...
    postGenerated: function (wo, done, test) {
        ...
        dbCon.getConnection(function (err, res) {
            ...
            con.query(sql, function (err, res) {
                ...
                sql += " SELECT DISTINCT " + wo.woId + " AS woId, calc.cunitId, calc.estimate, calc.done, 0 AS quantity";
                sql += " FROM";
                sql += " (SELECT pwl.pwId, cu.cunitId, cu.name, COALESCE(e.estimate,0) AS estimate, COALESCE(d.done,0) AS done";
                sql += " FROM pw_line AS pwl";
                sql += " LEFT JOIN cunit AS cu ON cu.cunitId = pwl.cunitId";
                sql += " LEFT JOIN (SELECT pwId, cunitId, SUM(quantity) AS estimate FROM pw_line GROUP BY pwId, cunitId)";
                sql += " AS e ON (e.pwId = pwl.pwId AND e.cunitId = cu.cunitId)";
                sql += " LEFT JOIN (SELECT pwId, cunitId, SUM(quantity) AS done FROM wo_line LEFT JOIN wo ON wo.woId = wo_line.woId GROUP BY pwId, cunitId)";
