...
var woAPI = {
    get: function (done, test) {
        dbCon.getConnection(function (err, res) {
            ...
            var sql = "SELECT wo.*, pw.name AS pwName, w.name AS workerName, pw.zoneId";
            sql += " FROM wo";
    ...
    postGenerated: function (wo, done, test) {
        ...
        dbCon.getConnection(function (err, res) {
            ...
            con.query(sql, function (err, res) {
                if (err) return done(err);
                wo.woId = res.insertId;
                sql = "INSERT INTO wo_line (woId, cunitId, estimate, done, quantity, chapterId)";
                sql += " SELECT DISTINCT " + wo.woId + " AS woId, calc.cunitId, calc.estimate, calc.done, 0 AS quantity, calc.chapterId";
                sql += " FROM";
                ...
                sql += " LEFT JOIN (SELECT pwId, cunitId, SUM(quantity) AS done FROM wo_line LEFT JOIN wo ON wo.woId = wo_line.woId GROUP BY pwId, chapterId, cunitId)";
