...
var itemStockAPI = {
    ...
    postUpdateStock: function (stk, done, test) {
        // Is there a record for that item and store?
        dbCon.getConnection(function (err, con) {
            ...
            con.query(sql, function (err, res) {
                ...
                if (res.length > 0 && res[0].lastInvDate) {
                    ...
                    itemStockAPI.postUpdateStockExists(stk, res[0].lastInvDate, function (err, res) {
                        if (err) return done(err);
                        done(null, null);
                    }, test);
                } else {
                    // new record or not inventoried
                    itemStockAPI.postUpdateStockNew(stk, function (err, res) {
                        if (err) return done(err);
                        done(null, null);
                    }, test);
                }
    ...
    postUpdateStockExists: function (stk, lastInvDate, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
    ...
    postUpdateStockNew: function (stk, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
