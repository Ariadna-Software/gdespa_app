...
var itemStockAPI = {
    ...
    postUpdateStockExists: function (con, stk, lastInvDate, done, test) {
        var sql = "SELECT " + stk.storeId + " AS storeId, it.itemId, (COALESCE(i.qin, 0) -  COALESCE(o.qout, 0)) AS stock, it.name";
        sql += " FROM item AS it";
        sql += " LEFT JOIN";
        ...
        con.query(sql, function (err, res) {
            ...
            if (calculatedStock < 0) {
                con.rollback(function () { });
                return done(new Error("El proceso dejaría el stock de artículo en negativo: " + res[0].name));
            }
    ...
    postUpdateStockNew: function (con, stk, done, test) {
        ...
        con.query(sql, function (err, res) {
            ...
            if (reg.stock < 0) {
                con.rollback(function () { });
                return done(new Error("No hay suficente stock del artículo: " + res[0].name));
            }
...
var fnStockDbToJs = function (odb) {
    ...
    o.lastInvDate = util.serverParseDate(o.lastInvDate);
    return o;
}
