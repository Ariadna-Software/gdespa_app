...
var docAPI = {
    ...
    getByPwId: function (id, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT d.*, pw.name as pwName, doc_type.name as typeName FROM doc As d LEFT JOIN pw ON pw.pwId = d.pwId LEFT JOIN doc_type ON doc_type.docTypeId = d.docTypeId WHERE d.pwId = ?";
            sql = mysql.format(sql, id);
            con.query(sql, function (err, res) {
                dbCon.closeConnection(con);
                if (err) return done(err);
