...
var inventoryAPI = {
    get: function (done, test) {
        dbCon.getConnection(function (err, res) {
            ...
            var sql = "SELECT inv.*, w.name AS workerName, s.name AS storeName, s.zoneId";
    ...
    putClose: function (inventory, done, test) {
        ...
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT COUNT(*) AS n FROM inventory_line";
            sql += " WHERE inventoryId =  ? AND newStock < 0";
            sql = mysql.format(sql, udb.inventoryId);
            con.query(sql, function (err, res) {
                if (err) return done(err);
                if (res[0].n > 0) {
                    return done(new Error('No se permiten inventarios con cantidades negativas'));
                }
                var sql = "UPDATE inventory SET ? WHERE inventoryId = ?";
                sql = mysql.format(sql, [udb, udb.inventoryId]);
                con.query(sql, function (err, res) {
