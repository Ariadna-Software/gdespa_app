...
var woWorkerAPI = {
    ...
    getByWoIdWorker: function (id, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT wow.*, w.name AS workerName";
    ...
    getByWoIdVehicle: function (id, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT wow.*, w.name AS workerName";
    ...
    put: function (woWorker, done, test) {
        ...
        dbCon.getConnection(function (err, res) {
            ...
            con.query(sql, function (err, res) {
                if (err) return done(err);
                if (res.length == 1) gdb.cost = res[0].cost;
                sql = "UPDATE wo_worker SET ? WHERE woWorkerId = ?";
...
var fnWoWorkerJsToDb = function (o) {
    ...
    var odb = o;
    odb.woWorkerId = o.id;
    if (o.wo) odb.woId = o.wo.id; // prevent error in delete
