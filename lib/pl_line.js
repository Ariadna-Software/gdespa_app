...
var plLineAPI = {
    get: function (done, test) {
        dbCon.getConnection(function (err, res) {
            ...
            var sql = "SELECT pll.plLineId, pll.plId, pll.cunitId, cu.name AS cunitName, pll.estimate, pll.done, pll.quantity, pll.unitCost,";
    ...
    getByPlId: function (id, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT pll.plLineId, pll.plId, pll.cunitId, cu.name AS cunitName, pll.estimate, pll.done, pll.quantity, pll.prevPlanned, pll.unitCost,";
            sql += " ch.order as chapterOrder, ch.name AS chapterName, ch.comments as chapterComments";
            sql += " FROM pl_line AS pll";
            sql += " LEFT JOIN pl ON pl.plId = pll.plId";
            sql += " LEFT JOIN cunit AS cu ON cu.cunitId = pll.cunitId";
    ...
    getByPlIdCost: function (id, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT COALESCE(SUM(quantity * unitCost),0) as TotalCost FROM pl_line as pl"
            sql += " WHERE pl.plId = ?";
            sql = mysql.format(sql, id);
            con.query(sql, function (err, res) {
                dbCon.closeConnection(con);
