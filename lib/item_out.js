...
var itemOutAPI = {
    get: function (done, test) {
        dbCon.getConnection(function (err, res) {
            ...
            var sql = "SELECT item_out.*, s.name AS storeName, w.name AS workerName, pw.name AS pwName, s.zoneId";
            sql += " FROM item_out";
    ...
    post: function (itemOut, done, test) {
        ...
        inventoryDb.checkInventoryAfter(itemOut.dateOut, itemOut.store.id, function (err, res) {
            if (err) return done(err);
            if (res) return done(new Error('Hay inventarios posteriores a esta fecha de salida'));
            // obtain db record
    ...
    postGenerated: function (itemOut, done, test) {
        // controls itemOut properties
        if (!fnCheckItemOut(itemOut)) {
            return done(new Error("Wrong object. You may be missing some properties"));
        }
        ...
        dbCon.getConnection(function (err, res) {
            ...
            con.query(sql, function (err, res) {
                if (err) return done(err);
                itemOut.itemOutId = res.insertId;
                sql = "INSERT INTO item_out_line (itemOutId, itemId, quantity)";
                sql += " SELECT " + itemOut.itemOutId + " AS itemOutId, cul.itemId, (cul.quantity * pwl.quantity) AS quantity";
                sql += " FROM pw";
                ...
                sql += " WHERE pw.pwId = ?";
                sql = mysql.format(sql, itemOut.pwId);
                con.query(sql, function (err, res) {
                    dbCon.closeConnection(con);
                    if (err) return done(err);
                    done(null, fnItemOutDbToJs(itemOut));
...
var fnItemOutDbToJs = function (odb) {
    ...
    delete o.workerName;
    delete o.pwName;
    // forma dates without time zone
    o.dateOut = util.serverParseDate(o.dateOut);
    return o;
}
