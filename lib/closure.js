    ...
    post: function (closure, done, test) {
        ...
        dbCon.getConnection(function (err, res) {
            ...
            con.query(sql, function (err, res) {
                ...
                sql += " SELECT ? AS closureId, pw.pwId, ROUND(COALESCE(d.c,0) / COALESCE(e.c,1), 2) AS estimate, ROUND(COALESCE(d.c,0) / COALESCE(e.c,1), 2) AS done, ROUND(COALESCE(d.c,0) , 2) AS amount";
                sql += " FROM pw";
                sql += " LEFT JOIN";
