...
// -------------------- MAIN API
var reportAPI = {
    getWorkerHours: function (initDate, endDate, workerId, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT w.name AS workerName, wok.woWorkerId, wo.woId, ";
            sql += " z.name AS zoneName, pw.name AS pwName,";
            sql += " wo.initDate AS workDate, wok.quantity, wok.normalHours, wok.extraHours";
            ...
            sql += " AND wo.initDate >= ? AND wo.initDate <= ?";
            sql = mysql.format(sql, [initDate, endDate]);
            if (workerId && workerId != 0) {
                sql += " AND w.workerId = ?";
                sql = mysql.format(sql, workerId);
            }
            sql += " ORDER BY w.name, wo.initDate, z.name, pw.name"
            con.query(sql, function (err, res) {
                dbCon.closeConnection(con);
                if (err) return done(err);
                var deliverys = fnWorkerHours(res, initDate, endDate);
                done(null, deliverys);
            });
        }, test);
    },
    ...
    getPwR1ById: function (id, done, test) {
        dbCon.getConnection(function (err, res) {
            ...
            sql += " pwl.pwLineId, cul.cunitLineId, pwl.chapterId, ch.order, ch.name AS chapterName, ch.comments AS chapterComments"
            sql += " FROM pw AS pw";
            ...
            sql += " WHERE pw.pwId = ?"
            sql += " ORDER BY ch.order, pwl.line, pwl.pwLineId, cul.line, cul.cunitLineId";
            sql = mysql.format(sql, id);
            con.query(sql, function (err, res) {
                dbCon.closeConnection(con);
                if (err) return done(err);
    ...
    getWo2ById: function (id, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT wo.woId, wo.initDate, w.name AS workerName, pw.name AS pwName, wo.comments, pw.reference,";
            sql += " wol.woLineId, cu.name AS cuName, wol.estimate, wol.done, wol.quantity";
            sql += " FROM wo";
            sql += " LEFT JOIN pw ON pw.pwId = wo.pwId";
            sql += " LEFT JOIN worker AS w ON w.workerId = wo.workerId";
            ...
            con.query(sql, function (err, res) {
                ...
                var pws = fnWoDbToJs(res);
                // obtain workers
                sql = "SELECT w.name AS worker, wok.quantity AS totalHours, wok.planHours, wok.normalHours, wok.extraHours,";
                sql += " wok.initialKm, wok.finalKm, wok.totalKm, wok.fuel"
                sql += " FROM wo AS wo";
                sql += " LEFT JOIN wo_worker AS wok ON wok.woId = wo.woId";
                sql += " LEFT JOIN worker AS w ON w.workerId = wok.workerId";
                sql += " WHERE wo.woId = ? AND w.resTypeId = 0";
                sql = mysql.format(sql, id);
                con.query(sql, function (err, res) {
                    if (err) return done(err);
                    pws.workers = res;
                    ...
                    sql += " LEFT JOIN wo_worker AS wok ON wok.woId = wo.woId";
                    sql += " LEFT JOIN worker AS w ON w.workerId = wok.workerId";
                    sql += " WHERE wo.woId = ? AND w.resTypeId = 1";
                    sql = mysql.format(sql, id);
                    con.query(sql, function (err, res) {
                        dbCon.closeConnection(con);
                        if (err) return done(err);
    ...
    getReportItemMain: function (id, id2, done, test) {
        async.waterfall([
            ...
            function (err, results) {
                if (err) return done(err);
                if (!results) return done(null, null);
                // provisional, first object
                if (!results[0]) return done(null, null);
                var o = results[0];
                o.nreport = "MOVIMIENTOS DE ARTÍCULO";
                o.ilines = results[1];
                done(null, o);
            });
    },
    getInfWorkerDays: function (beginDate, endDate, workerId, done, test) {
        dbCon.getConnection(function (err, res) {
            if (err) return done(err);
            var con = res; // mysql connection
            var sql = "SELECT * FROM worker WHERE active = 1 AND administrative = 0 AND resTypeId = 0";
            if (workerId && workerId != 0) {
                sql += " AND workerId = ?";
                sql = mysql.format(sql, workerId);
            }
            ...
            con.query(sql, function (err, res) {
                dbCon.closeConnection(con);
                if (err) return done(err);
                fnInfDaysWorkers(res, beginDate, endDate, function (err, results) {
                    if (err) return done(err);
                    fs.writeFileSync('/tmp/jsHoursDays.json', JSON.stringify(results, null, 2));
                    done(null, results);
                });
            });
        }, test);
...
var fnPwReportStatusMain = function (id, done) {
    dbCon.getConnection(function (err, res) {
        if (err) return done(err);
        var con = res; // mysql connection
        var sql = "SELECT DATE_FORMAT(CURDATE(),'%d/%m/%Y') AS reportDate,";
...
var fnReportStoreMain = function (id, done) {
    dbCon.getConnection(function (err, res) {
        if (err) return done(err);
        var con = res; // mysql connection
        var sql = "SELECT *, DATE_FORMAT(CURDATE(),'%d/%m/%Y') AS reportDate from store";
        sql += " WHERE storeId = ?"
        sql = mysql.format(sql, id);
        con.query(sql, function (err, res) {
            dbCon.closeConnection(con);
...
var fnReportItemLine = function (id, id2, date, done) {
    ...
    dbCon.getConnection(function (err, res) {
        ...
        sql += " FROM";
        sql += " (SELECT io.itemOutId as id, iol.itemOutLineId AS id2, iol.itemId, dateOut AS dMov, 'SALIDA' AS TYPE,  w.name AS inCharge, quantity";
        ...
        sql += " UNION";
        sql += " SELECT ii.itemInId as id, iil.itemInLineId AS id2, iil.itemId, dateIn AS dMov, 'ENTRADA' AS TYPE,  w.name AS inCharge, quantity";
...
var fnWorkerHours = function (odb, initDate, endDate) {
    var o = {
        nreport: "HORAS POR TRABAJADOR",
        initDate: moment(initDate).format('DD/MM/YYYY'),
    ...
    var prevWorkerName = "";
    var workerHours = 0;
    var workerNormalHours = 0;
    var workerExtraHours = 0;
    var worker = {};
    odb.forEach(function (w) {
        if (prevWorkerName != w.workerName) {
            if (prevWorkerName != "") {
                worker.totalHours = workerHours;
                worker.totalNormalHours = workerNormalHours;
                worker.totalExtraHours = workerExtraHours;
            ...
            workerNormalHours = 0;
            workerExtraHours = 0;
            prevWorkerName = w.workerName;
        }
        line = {
            woId: w.woId,
            woWorkerId: w.woWorkerId,
            date: moment(w.workDate).format('DD/MM/YYYY'),
            zone: w.zoneName,
            pw: w.pwName,
            quantity: w.quantity,
        ...
        worker.lines.push(line);
        workerHours += w.quantity;
        workerNormalHours += w.normalHours;
        workerExtraHours += w.extraHours;
    });
    worker.totalHours = workerHours;
    worker.totalNormalHours = workerNormalHours;
    worker.totalExtraHours = workerExtraHours;
...
var fnPwR1DbToJs = function (odb) {
    ...
    var l2 = null;
    odb.forEach(function (v) {
        // change chapter and is not first
        if (v.chapterId != prevChapterId) {
            if (ch) {
                o.chapters.push(ch);
            }
            ch = {
                order: v.order,
                name: v.chapterName,
                comments: v.chapterComments,
                lines: []
            };
            prevChapterId = v.chapterId;
            prevPwLineId = 0;
        }
        ...
        if (v.pwLineId != prevPwLineId) {
            ...
            l1 = {
                ...
                quantity: v.pwlQ,
                k: v.pwlK,
                ck: util.roundToTwo(v.pwlK * v.pwlC),
                amount: v.pwlA,
                comments: v.comments,
                items: []
            };
...
var fnInfDaysWorkers = function(workers, startDate, endDate, done){
    var reportInf = [];
    async.eachSeries(workers, function(w, callback){
        infOfAworker(startDate, endDate, w.workerId, w.name, function(err, res){
            if (err) return callback(err);
            reportInf = reportInf.concat(res);
            callback();
        });
    }, function(err){
        if (err) return done(err);
...
var infOfAworker = function(startDate, endDate, workerId,workerName, done){
    var infOfWorker = [];
    var daysRange = rangeOfDays(startDate, endDate);
    async.eachSeries(daysRange, function(day, callback){
        infW = {
            day: day,
            worker: workerName,
            woHours: 0,
        ...
        fnWoWorkedHoursInDay(workerId, day, function(err, res){
            if (err) return callback(err);
            if (res != null) infW.woHours = res.quantity;
            fnMoWorkedHoursInDay(workerId, day, function(err, res){
                if (err) return callback(err);
                if (res != null) infW.moHours = res.quantity;
                fnTypeOfDay(workerId, day, function(err, res){
                    if (err) return callback(err);
                    infW.infDay = res;
                    if ((infW.moHours + infW.woHours) == 0 && infW.infDay == "LABORABLE"){
                        infW.nsHours = 8;
                    }
                    infOfWorker.push(infW);
                    callback();
                });
            });
        });
        ...
        if (err) return done(err);
        done(null, infOfWorker);
    });
}

var rangeOfDays = function (startDate, endDate) {

    var retVal = [];
    var current = moment(startDate).format("YYYY-MM-DD");
...
var fnMoWorkedHoursInDay = function (workerId, moDate, done) {
    dbCon.getConnection(function (err, res) {
        if (err) return done(err);
        var con = res; // mysql connection
        var sql = "SELECT mol.moWorkerId, mo.initDate, quantity ";
        sql += " FROM mo_worker AS mol";
        sql += " LEFT JOIN mo ON mo.moId = mol.moId";
        sql += " WHERE mol.workerId = ? AND mo.initDate = ?"
        sql = mysql.format(sql, [workerId, moDate]);
        con.query(sql, function (err, res) {
            dbCon.closeConnection(con);
            if (err) return done(err);
            if (res.length == 0) {
...
var fnWoWorkedHoursInDay = function (workerId, woDate, done) {
    dbCon.getConnection(function (err, res) {
        if (err) return done(err);
        var con = res; // mysql connection
        var sql = "SELECT wol.woWorkerId, wo.initDate, quantity ";
        sql += " FROM wo_worker AS wol";
        sql += " LEFT JOIN wo ON wo.woId = wol.woId";
        sql += " WHERE wol.workerId = ? AND wo.initDate = ?"
        sql = mysql.format(sql, [workerId, woDate]);
        con.query(sql, function (err, res) {
            dbCon.closeConnection(con);
            if (err) return done(err);
            if (res.length == 0) {
...
var fnTypeOfDay = function(workerId, workDate, done){
    var wId = workerId;
    var wDate = workDate;
    async.series(
        [
            // Is Sunday?
            function(callback){
                if (new Date(wDate).getDay() == 0){
                    return callback(null, "DOMINGO")
                } else {
                    return callback(null, "LABORABLE");
                }
            },
            // Is holiday?
            function(callback){
                dbCon.getConnection(function (err, res) {
                    if (err) return done(err);
                    var con = res; // mysql connection
                    var sql = "SELECT * FROM holiday WHERE holidayDate = ?"
                    sql = mysql.format(sql, wDate);
                    con.query(sql, function (err, res) {
                        dbCon.closeConnection(con);
                        if (err) return callback(err);
                        if (res.length == 0) {
                            callback(null, "LABORABLE");
                        } else {
                            callback(null, "FESTIVO: " + res[0].name);
                        }
                    });
                }, false);
            },
            ...
            function(callback){
                dbCon.getConnection(function (err, res) {
                    if (err) return done(err);
                    var con = res; // mysql connection
                    var sql = "SELECT * FROM `abs` AS ab ";
                    sql += " LEFT JOIN abs_type AS abt ON abt.absTypeId = ab.absTypeId";
                    sql += " WHERE ab.fromDate <= ? AND ab.toDate >= ?";
                    sql += " AND workerId = ?"
                    sql = mysql.format(sql, [wDate, wDate, wId]);
                    con.query(sql, function (err, res) {
                        dbCon.closeConnection(con);
                        if (err) return callback(err);
                        if (res.length == 0) {
                            ...
                            callback(null, "AUSENCIA JUSTIFICADA: " + res[0].name);
                        }
                    });
                }, false);
            }
        ...
        function(err, results){
            if (err) return done(err);
            var infDay = "LABORABLE"
            results.forEach(function(d){
                if (d != infDay && infDay == "LABORABLE"){
                    infDay = d;
                } 
            })
            return done(null, infDay);
        }
    )
}
