...
var obtainReport = function () {
    if (!datosOK()) return;
    var file = "../reports/mo_resumido.mrt";
    if (vm.detalle()) {
        file = "../reports/mo_detallado.mrt";
    }
    ...
    report.dataSources.items[0].sqlCommand = rptMoDetail(sql);
...
var rptMoDetail = function (sql) {
    if (vm.detalle()) {
        // informe detallado
        if (vm.sZona()) {
            sql += " AND mo.zoneId = " + vm.sZona();
        }
        if (vm.sBrigada()) {
            sql += " AND mo.teamId = " + vm.sBrigada();
        }
    } else {
        // informe resumido GROUP BY 1,2,3,4
        var sql2 = "";
        if (vm.sZona()) {
            sql2 += " AND mo.zoneId = " + vm.sZona();
        }
        if (vm.sBrigada()) {
            sql2 += " AND mo.teamId = " + vm.sBrigada();
        }       
        sql2 += "  GROUP BY 1,2,3,4";
        sql = sql.replace("GROUP BY 1,2,3,4", sql2);
    }
    return sql;
}
